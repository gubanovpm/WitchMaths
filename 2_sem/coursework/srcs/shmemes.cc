...
void WitchMath::get_args(const int argc, const char *argv[], matrix_t **matrix) noexcept {
	if (argc != 5) {
		...
		std::cout << "<k>    -- count of steps on r coord" << std::endl;
...
void matrix_t::scheme_ro(const size_t i, const double c) noexcept {
    ...
    ro[i+1][0] = k/2 * (
        (k - 1) * std::pow(h, 2) * ro[i][1] * u[i][1] -
        2 * std::pow(h, 2) * ro[i][0] * u[i][0]
    ) + ro[i][0];
}
//===============================================================================================
//===============================================================================================
...
void matrix_t::view_plot(const size_t plot_num) noexcept {
    ...
    std::string name;
    switch (plot_num) {
        case 0: { data = &ro; name = "ro(t, r)"; break;}
        case 1: { data =  &u; name = "u(t, r)"; break;}
        case 2: { data =  &E; name = "E(t, r)"; break;}
        case 3: { data =  &P; name = "P(t, r)"; break;}
        default: {
